...
The runner also gives child processes an explicit allowlist instead of inheriting the parent shell environment. It keeps only basic process, locale, terminal, and certificate variables; blocks GitHub tokens, API keys, generic secret variables, and Git repository redirection variables; disables system and global Git configuration for the temporary repository; and tells model-generated shell commands to inherit no environment variables. Authentication must therefore already be available through the runtime's browser or system credential storage under the retained home directory, not through an environment API key. The result summary reports `"environment": "allowlisted"` when this boundary is active.
