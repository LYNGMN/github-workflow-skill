...
행동하기 전에는 위의 영어 원어(English term)를 먼저 읽고, 아래의 한국어 설명으로 같은 결정 용어를 확인합니다.

### 리포지터리

리포지터리(Repository)는 프로젝트 파일과 그 파일이 어떻게 바뀌었는지를 기록한 이력을 함께 보관하는 작업 단위입니다.

### 원격 저장소

원격 저장소(Remote)는 GitHub처럼 다른 위치에 있는 리포지터리입니다. 내 컴퓨터의 리포지터리와 커밋을 주고받을 수 있습니다.

### 인증

인증(Authentication)은 도구가 현재 어느 GitHub 계정을 사용할 수 있는지 확인하는 일입니다. 도구가 설치되어 있어도 인증된 것은 아니며, 브라우저 로그인이 성공해도 모든 실행 환경에서 같은 인증정보를 사용할 수 있다고 단정할 수 없습니다.

### 권한 승인

권한 승인(Authorization)은 특정 계정 범위나 행동을 사용자가 허용하는 일입니다. 인증이 성공해도 푸시, 풀 리퀘스트 수정, 리뷰 준비 완료(Ready for review), 병합, 배포, 삭제, 계정 권한 변경이 자동으로 승인되지는 않습니다.

### OAuth 권한 범위

OAuth 권한 범위(OAuth Scope)는 인증정보가 사용할 수 있는 계정 기능의 범위를 설명합니다. 현재 리포지터리 밖에도 영향을 줄 수 있으므로 승인 전에 표시된 계정과 권한 범위를 확인하고, 토큰(token)이나 일회용 기기 코드를 노출하지 않습니다.

### 클론

클론(Clone)은 원격 리포지터리의 파일과 이력을 내 컴퓨터에 복사해 작업을 시작하는 방법입니다.

### 풀

풀(Pull)은 원격 저장소의 커밋된 변경을 내 작업 환경으로 가져오는 일입니다. 풀 리퀘스트를 만드는 일과는 다릅니다.

### 푸시

푸시(Push)는 내 컴퓨터에 있는 커밋을 원격 저장소로 보내는 일입니다. 병합이나 배포(deployment)가 자동으로 이루어진다는 뜻은 아닙니다.

### 브랜치

브랜치(Branch)는 독립된 작업 흐름에 붙이는 이름입니다. 한 가지 목적을 위해 `<owner>/<type>/<purpose>` 형식을 사용합니다.

### 워크트리

워크트리(Worktree)는 같은 리포지터리를 다른 폴더에 한 번 더 꺼내 놓은 작업 공간입니다. 현재 폴더를 바꾸지 않고 다른 브랜치를 작업할 수 있습니다.

### 스테이지

스테이지(Stage)는 다음 커밋에 담을 파일 변경만 골라 둔 상태입니다. 폴더의 모든 변경이 자동으로 포함되지는 않습니다.

### 커밋

커밋(Commit)은 한 가지 완료된 목적을 되돌아볼 수 있게 저장한 기록 지점입니다. 브랜치 이름만으로는 이런 복구 지점이 만들어지지 않습니다.

### 이슈

이슈(Issue)는 문제, 요청, 결정 사항과 완료 조건을 검색 가능하게 남기는 기록입니다. 비슷한 이슈가 있는지 먼저 찾습니다.

### 풀 리퀘스트

풀 리퀘스트(Pull Request)는 브랜치 변경을 병합하기 전에 검토해 달라고 요청하는 기록입니다. 풀과는 다른 작업입니다.

### 드래프트 풀 리퀘스트

초안 풀 리퀘스트(Draft Pull Request)는 아직 병합 준비가 되지 않은 진행 중인 풀 리퀘스트입니다. 이 패키지는 모든 풀 리퀘스트를 초안(Draft)으로 시작합니다.

### 리뷰

리뷰(Review)는 특정 헤드 SHA(Head SHA)에 대해 요구사항, 범위, 품질, 보안, 테스트, 운영 위험을 확인하는 일입니다. 변경 뒤에는 새 SHA를 다시 검토합니다.

### 병합

병합(Merge)은 승인된 풀 리퀘스트의 변경을 기준 브랜치(Base Branch)에 반영하는 일입니다. 병합 전에는 별도의 최종 승인이 필요합니다.

### 스쿼시 병합

스쿼시 병합(Squash Merge)은 풀 리퀘스트의 여러 커밋을 하나의 최종 커밋으로 합쳐 병합하는 방법입니다. 이 패키지에서는 이 방법만 허용합니다.

### SHA

SHA는 특정 커밋을 가리키는 고유 식별자입니다. 리뷰 승인은 검토한 정확한 헤드 SHA에만 적용됩니다.

### 레이블

레이블(Label)은 이슈와 풀 리퀘스트를 분류하고 찾기 쉽게 만드는 구조화된 표시입니다. 제목에 같은 상태를 반복하지 않아도 됩니다.

### 담당자

담당자(Assignee)는 이슈나 풀 리퀘스트를 다음 단계로 진행할 책임이 있는 사람입니다. 책임자가 불분명하면 작업을 시작하지 않습니다.

### 완료 조건

완료 조건(Acceptance Criteria)은 이슈가 끝났다고 판단할 수 있는 관찰 가능한 기준입니다. 모호하면 작업 범위를 확정하지 않습니다.

### 라이선스

라이선스(License)는 다른 사람이 패키지를 어떻게 사용할 수 있는지와 지켜야 할 의무를 설명합니다. [MIT License](../LICENSE)는 사용, 복사, 수정, 배포, 재라이선스, 판매를 허용합니다. 배포할 때는 저작권과 라이선스 고지문을 남겨야 하며, 품질이나 특정 목적 적합성을 보증하지 않습니다. [SPDX MIT 페이지](https://spdx.org/licenses/MIT.html)에서 식별자를 확인할 수 있습니다.
